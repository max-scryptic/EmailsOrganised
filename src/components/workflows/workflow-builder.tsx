...
const defaultClassifierPosition = { x: 480, y: 300 };
    ...
    canvasEdges.forEach((edge) => {
      ...
      if (marker) {
        marker.style.transform = canvasPositionTransform(
          edgeMarkerPoint(from, to, edge.fromBranchIndex)
        );
      }
    });
  }
            ...
            {canvasEdges.map((edge) => {
              ...
              return (
                <WireFilterMarker
                  ...
                  filter={edge.filter}
                  position={edgeMarkerPoint(from, to, edge.fromBranchIndex)}
...
function edgePathDefinition(
  ...
  return edgePoints(fromNode, toNode, fromBranchIndex)
    .map(
      (point, index) =>
...
/**
 * The corners the wire turns, from its outlet to its inlet. Split out from the
 * `d` string because the filter marker has to sit on the drawn wire, and the
 * only way to know where that is is to walk the same points the path is built
 * from.
 */
function edgePoints(
  fromNode: { position: CanvasNodePosition; height: number },
...
/**
 * Where a wire carries its filter marker: the middle of its longest horizontal
 * run.
 *
 ...
 * along and still be read off unambiguously.
 */
function edgeMarkerPoint(
  ...
  const points = edgePoints(fromNode, toNode, fromBranchIndex);
  const runs = points
    .slice(1)
    .map((end, index) => ({ start: points[index], end }))
    .filter((leg) => leg.start.y === leg.end.y)
    .sort(
  ...
  return longest
    ? { x: (longest.start.x + longest.end.x) / 2, y: longest.start.y }
    : points[0];
}
...
function wireAnchor(
  ...
  return {
    position: edgeMarkerPoint(fromNode, toNode, fromBranchIndex),
    width: 0,
    height: 0,
  };
}
