...
function systemPrompt() {
  ...
  return [
    ...
    "ASKING FOR EXAMPLES. When your reply asks for example emails, set `needsExamples` to true and set it to false on every other turn. The chat answers that turn with a form (a subject and a body per email, and a button for adding the next), so ask for the emails themselves, not for a description of them, and ask once rather than chasing a second round. The form is only offered on a turn you set this on, so a turn that asks for examples in words and leaves it false asks for something the user cannot give you. Do NOT ask for counter-examples: emails that look similar but should be left alone. The form has its own button for those, so the user adds them if they think the distinction matters. If some do arrive, use them; never ask for more.",

    "WHEN TO ASK. Examples are what the sorting instruction is written from, so ask for them once you know which emails the user cares about, and do not set `ready` before they have arrived. Ask for them on their own turn: a reply that asks for an address and for example emails at once gets one of the two answered. Do not ask on the opening turn, before the user has said what they want caught, and do not ask again about mail you already have examples of.",
